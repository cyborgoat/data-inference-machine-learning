...
\begin{document}
	...
	\author{Junxiao Guo \\ Andrew ID: junxiaog \\\\ Programming Language:Python \\\\ Libraries: numpy, pandas, seaborn, matplotlib, statemodels, sklearn}
	...
	\answer{
		Reference:
		\begin{itemize}
			\item \url{https://en.wikipedia.org/wiki/Rule-based_system}
		\end{itemize}
	}
	\questiontext{Domain knowledge required to establish a rule}
	\answer{
		Yes, becasue rule-based approach methods must have a domain knowledge in order to establish a approriate rule.
	}
	...
	\answer{
		Reference:
		\begin{itemize}
			\item {https://www.investopedia.com/terms/o/overfitting.asp}
		\end{itemize}
	}
...
\answer{
	...
	\begin{itemize}
		\item 	Multi-column Deep Neural Networks for Image Classiﬁcation on MNIST Dataset has a 0.23\% benchmark error rate.
		\item 	Stanford Tagger 1.0 using Maximum entropy cyclic dependency network	has a benchmark accuracy of 97.24\% per token on all tokens on WSJ Dataset.
	\end{itemize}}
	\answer{
		Reference:
		\begin{itemize}
			\item {https://www.quora.com/What-is-meant-by-a-benchmark-solution-in-machine-learning}
		\end{itemize}}
	...
	\answer{
		Reference:
		\begin{itemize}
		\item {https://towardsdatascience.com/accuracy-precision-recall-or-f1-331fb37c5cb9}
		\end{itemize}
	}
	\answer{
	...
	\answer{
		Reference:
		\begin{itemize}
			\item {https://www.britannica.com/topic/collinearity-statistics}
		\end{itemize}
	}
	\newpage
	...
	\begin{itemize}
		\item 'Age', 'S4', 'S5', 'S6'	
	\end{itemize}
	And this could be a problem of collinearity.
	...
	\answer{
		...
		\end{itemize}
		\underline{How does this function work}
		\begin{itemize}
			\item By using stepwise method, the function firtly build a fully model then drop the less significant variable one by a time, then it twicks the parameters untils the final feature numbers not change.
		\end{itemize}
	...
	\answer{
		Reference:
		\begin{itemize}
			\item {https://stats.stackexchange.com/questions/29325/what-is-the-difference-between-linear-regression-and-logistic-regression}
		\end{itemize}
	}
	...
	\answer{
			\begin{tabular}{|l|l|}
			\hline
			\textbf{Feature}                    & \textbf{Survival Probability} \\ \hline
			Children(0-14 Years)       & 0.5700934579439252   \\ \hline
			Youth(15-24 Years)         & 0.3879598662207358   \\ \hline
			Adults(25-64 Years)        & 0.3974358974358974   \\ \hline
			Senior(65 Years and older) & 0.15384615384615385  \\ \hline
			& \\ \hline
			Passenger Class 1          & 0.6191950464396285   \\ \hline
			Passenger Class 2          & 0.4296028880866426   \\ \hline
			Passenger Class 3          & 0.2552891396332863   \\ \hline
			& \\ \hline
			Gender: Male               & 0.19098457888493475  \\ \hline
			Gender: Female             & 0.7274678111587983   \\ \hline
			\end{tabular}
	}
	\newpage
